...
LocaisController.$inject = ['$stateParams', '$state', '$http', '$q', 'PaginationService', 'spinnerService'];

function LocaisController($stateParams, $state, $http, $q, paginationService, spinnerService) {
    var vm = this;
    ...
    vm.init = function () {
        ...
        if ($stateParams.nome || $stateParams.capacidade || $stateParams.endereco || $stateParams.excluidos || $stateParams.page || $stateParams.pageSize) {

            spinnerService.show('loader');

            vm.query = {
                'nome': $stateParams.nome,
                'capacidade': parseInt($stateParams.capacidade),
                'endereco': $stateParams.endereco,
    ...
    vm.filtrar = function (page) {
        ...
        $state.go('locais',
            {
                'nome': vm.query.nome,
                'capacidade': vm.query.capacidade,
                'endereco': vm.query.endereco,
    ...
    vm.consultar = function (page) {
        ...
        return $http.get("/locais/quantidade", { params: vm.query })
            .then(success, error);
    };
    ...
    var buscar = function () {
        ...
        return $http.get("/locais/buscar", { params: vm.query }).then(success, error).finally(onComplete);
    };
    ...
    vm.excluir = function (id) {
        ...
        var error = function (response) {
            ...
            console.log('Erro ao excluir local id = ' + vm.registroParaExcluir);
            vm.registroParaExcluir = null;
        };

        spinnerService.show('loader');

        return $http.delete('/locais/excluir/' + vm.registroParaExcluir).then(success, error).finally(onComplete);
    };
