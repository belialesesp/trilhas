angular.module('trilhasapp')
    .directive('pisPasepValido', function () {
        return {
            restrict: 'A',
            require: 'ngModel',
            link: function (scope, elem, attrs, ctrl) {
                scope.$watch(attrs.ngModel, function () {
                    if (elem[0].value.length === 0) {
                        ctrl.$setValidity('pisPasepValido', true);
                    } else if (elem[0].value.length < 11) {
                        ctrl.$setValidity('pisPasepValido', false);
                    } else {
                        ...
                        // Retira a mascara
                        var numeroPIS = elem[0].value.replace(/[^\d]+/g, '');
                        ...
                        if (numeroPIS.length !== 11 ||
                            ...
                            ctrl.$setValidity('pisPasepValido', resto === digito);
                        }
                    }
                });
            }
